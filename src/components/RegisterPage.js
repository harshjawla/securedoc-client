...
export default function RegisterPage() {
    ...
    if(redirect===1){
        return <Navigate to="/login" state={{message: "Registration Successful, please login", color: "blue"}} />
    } else if(redirect===2){
        setLoading(false);
        return <Navigate to="/register" state={{message: "Falied to register, please retry", color: "red"}}/>
    } else if(redirect===3){
        return <Navigate to="/login" state={{message: "Already Registered, please login", color: "blue"}} />
    }
    ...
    return (
        <>
        {state && (
            <div className={`bg-${state.color}-500 py-2 px-4 rounded-md text-white text-center fixed top-4 right-4 flex gap-4`}>
            <p>{state.message}</p>
